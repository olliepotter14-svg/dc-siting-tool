...
NAME_MAP = {
    ...
    # "iron acton" — REMOVED: NGED LTDS has 'iron acton' directly (firm=3888 MW,
    #   util 19.6% → 53.6% by 2029); the old "(SWe)" override was sending us to
    #   NGED technical_limit_only data and bypassing the better LTDS source.
    "coventry":             "coventry",
    "bushbury":             "bushbury",
    "feckenham":            "feckenham",
    ...
    # NOTE: 'seabank' and 'rugeley' overrides REMOVED — NGED LTDS has both as
    # direct GSPs now, removing the override lets resolve() reach the better data.
    "walham":               "iron acton swe",   # NGED Iron Acton (SWe) 27.6km — Gloucester 400kV (no LTDS match)
    "corby":                "grendon",          # NGED Grendon 15.6km — now LTDS-backed
    "high marnham":         "staythorpe",       # NGED Staythorpe 10.6km — now LTDS-backed
    "cottam":               "staythorpe",       # NGED Staythorpe 19.2km — now LTDS-backed
    "cilfynydd":            "upper boat",       # NGED Upper Boat 3.1km — now LTDS-backed
    "imperial park":        "uskmouth",         # NGED Uskmouth 8.7km — now LTDS-backed
    "spalding north":       "walpole",          # NGED Walpole 2.4km — now LTDS-backed
    "hinkley point":        "taunton",          # NGED Taunton 23.3km — Bridgwater/Taunton in LTDS
...
# ── Load UKPN ───────────────────────────────────────────────────────────
def load_ukpn_ltds() -> dict:
    """
    Load UKPN LTDS Table 3a demand headroom data.

    This is the authoritative source for UKPN substation headroom — firm capacity
    under N-1 security standards minus committed peak demand. Replaces the
    asset_limit_proxy calculation (nameplate - max observed) which overstates
    headroom at congested urban substations like City Road and St Johns Wood.

    Generated by: python3 scripts/fetch_ukpn_ltds.py --api-key KEY
                  or:  python3 scripts/fetch_ukpn_ltds.py --from-file data/ukpn_ltds_table3a_raw.csv
    """
    ltds_file = DATA_DIR / "ukpn_ltds_headroom.json"
    if not ltds_file.exists():
        print(f"  UKPN LTDS: {ltds_file} not found — skipping (run fetch_ukpn_ltds.py first)")
        return {}
    with open(ltds_file) as f:
        recs = json.load(f)
    lookup = {}
...
def load_ssen_ltds() -> dict:
    """
    SSEN LTDS-style headroom from already-on-disk headroom dashboard, using
    'Contracted Demand' columns as the committed pipeline.

    Generated by: python3 scripts/fetch_ssen_ltds.py
    """
    f_path = DATA_DIR / "ssen_ltds_headroom.json"
    if not f_path.exists():
        print(f"  SSEN LTDS: {f_path} not found — skipping")
        return {}
    ...
    for key, d in recs.items():
        lookup[key] = {
            "headroom_mw":        d.get("headroom_mw"),
            "capacity_mw":        d.get("firm_capacity_mw"),
            "max_demand_mw":      d.get("peak_demand_mw"),
            "ltds_utilisation":   d.get("utilisation_pct"),
            "committed_util":     d.get("committed_util_pct"),
            "committed_headroom": d.get("committed_headroom_mw"),
            "source":             d.get("source"),
            "quality":            "direct_headroom",
            "dno":                "SSEN",
            "raw_name":           d.get("gsp_raw_name", key),
        }
    print(f"  SSEN LTDS-style: {len(lookup)} GSPs loaded (firm − peak; contracted demand as floor)")
    return lookup


def load_npg_ndp() -> dict:
    """
    NPg LTDS-style headroom from NDP Demand Headroom dataset.

    Uses 2030 demand headroom (DFES Holistic Transition scenario) as the
    forward-conservative headroom value, with committed-utilisation derived
    where firm capacity is available.

    Generated by: python3 scripts/fetch_npg_ndp.py
    """
    f_path = DATA_DIR / "npg_ndp_committed.json"
    if not f_path.exists():
        print(f"  NPg NDP: {f_path} not found — skipping")
        return {}
    with open(f_path) as f:
        recs = json.load(f)
    lookup = {}
    for key, d in recs.items():
        # Use 2030 (post-pipeline) headroom as the headroom basis — this is
        # the demand-side queue-floored view, baked directly into headroom.
        hr_2030 = d.get("committed_headroom_mw")
        lookup[key] = {
            "headroom_mw":        round(hr_2030, 1) if hr_2030 is not None else None,
            "capacity_mw":        d.get("firm_capacity_mw"),
            "max_demand_mw":      d.get("peak_demand_mw"),
            "ltds_utilisation":   d.get("utilisation_pct"),
            "committed_util":     d.get("committed_util_pct"),
            "committed_headroom": hr_2030,
            "source":             d.get("source"),
            "quality":            "direct_headroom",
            "dno":                "NPG",
            "raw_name":           d.get("gsp_raw_name", key),
        }
    print(f"  NPg NDP committed: {len(lookup)} GSPs loaded (2030 forward headroom — Holistic Transition)")
    return lookup


def load_enw_heatmap() -> dict:
    """
    ENW per-GSP demand headroom from the May 2026 Heatmap Excel workbook —
    replaces the PRY-aggregate coordinate-match proxy we used to use.

    Generated by: python3 scripts/fetch_enw_heatmap.py
    """
    f_path = DATA_DIR / "enw_heatmap_headroom.json"
    if not f_path.exists():
        print(f"  ENW heatmap: {f_path} not found — skipping")
        return {}
    with open(f_path) as f:
        recs = json.load(f)
    lookup = {}
    ...
    print(f"  ENW heatmap: {len(lookup)} GSP entries loaded (firm non-inverter demand headroom)")
    return lookup


def _norm_ea(s: str) -> str:
    """Match the normalisation used by fetch_ea_register.py."""
    import re as _re
    n = (s or "").lower().strip()
    n = _re.sub(r"\s+\d+(?:\s*/\s*\d+)*\s*k?v\b", " ", n)
    for suf in [
...
def apply_ea_register(subs: list) -> int:
    """
    Attach NESO EA Register aggregates to each substation and use the
    demand-side committed MW as a queue-pressure floor.

    Strategy:
      1. Load the per-connection-point aggregates produced by
         fetch_ea_register.py.
      2. For each substation, name-match against EA keys using:
         - exact normalised match
         - longest-prefix or longest-contains match (so 'Hunterston'
           catches 'hunterston east' and 'hunterston 400 132'; we then
           AGGREGATE all such matches because the EA register splits one
           physical sub across multiple verbose connection-point names).
      3. Write fields:
           ea_total_mw, ea_demand_mw, ea_generation_mw, ea_battery_mw,
           ea_project_count, ea_earliest_date, ea_latest_date,
           ea_matched_names (list)
      4. If estimated_headroom_mva > 0, compute ea_queue_pressure_pct
         from total_mw / headroom × 100 and apply as a queue floor (max
         with any existing tec/ltds floor). The downstream recalculation
         loop will pick this up via real_queue_pressure_pct.

    Returns count of substations that got at least one EA match.
    """
    ea_file = DATA_DIR / "ea_register_by_sub.json"
    if not ea_file.exists():
        print(f"  EA register: {ea_file} not found — skipping (run fetch_ea_register.py first)")
        return 0
    ...
    attached = 0
    for sub in subs:
        sn = _norm_ea(sub["name"])
        if not sn:
            continue

        # Collect all matching EA entries: exact + any key that starts with
        # or contains the substation name as a whole word. We use whole-word
        # matching (with leading/trailing word boundaries) to avoid false
        # positives like "norton" matching "thornton".
        matches = []
        for k, v in ea.items():
            if k == sn:
                matches.append((k, v))
                continue
            # Whole-word containment: either sn is a word prefix of k, or
            # k is a word prefix of sn, OR sn appears as a complete word
            # within k.
            sn_words = sn.split()
            k_words  = k.split()
            if (k_words[:len(sn_words)] == sn_words   # k starts with sn
                or sn_words[:len(k_words)] == k_words):  # sn starts with k
                matches.append((k, v))

        if not matches:
            continue

        # Aggregate across all matching connection points
        total_mw      = sum(v["total_mw"]      for _, v in matches)
        demand_mw     = sum(v["demand_mw"]     for _, v in matches)
        generation_mw = sum(v["generation_mw"] for _, v in matches)
        battery_mw    = sum(v["battery_mw"]    for _, v in matches)
        project_count = sum(v["project_count"] for _, v in matches)
        earliest = min((v["earliest"] for _, v in matches if v["earliest"]), default=None)
        latest   = max((v["latest"]   for _, v in matches if v["latest"]),   default=None)

        sub["ea_total_mw"]      = round(total_mw, 1)
        sub["ea_demand_mw"]     = round(demand_mw, 1)
        sub["ea_generation_mw"] = round(generation_mw, 1)
        sub["ea_battery_mw"]    = round(battery_mw, 1)
        sub["ea_project_count"] = project_count
        sub["ea_earliest_date"] = earliest
        sub["ea_latest_date"]   = latest
        sub["ea_matched_names"] = sorted({n for _, v in matches for n in v["raw_names"]})
        attached += 1

        # Compute EA-derived queue pressure where headroom is known.
        # Use total (gen + storage + demand) because all queued MW competes
        # for substation reinforcement budget — though the demand subset is
        # what specifically competes with a new DC load.
        hr = sub.get("estimated_headroom_mva")
        if hr is not None and hr > 0:
            ea_q_pct = round((total_mw / hr) * 100, 1)
            sub["ea_queue_pressure_pct"] = ea_q_pct
            # Apply as queue floor on top of any TEC/LTDS floor
            cur = sub.get("real_queue_pressure_pct", 0) or 0
            sub["real_queue_pressure_pct"] = max(cur, ea_q_pct)

    return attached


def load_nged_ltds() -> dict:
    """
    Load NGED LTDS Table 3a equivalent (firm capacity − current peak demand,
    plus 5-year committed-pipeline forecast).

    Same schema and role as load_ukpn_ltds() — overrides the NGED
    technical_limit_only entries with real demand-side headroom and feeds
    a committed-utilisation floor into queue-pressure scoring (process
    chain: see ltds_floor logic at the foot of main()).

    Generated by: python3 scripts/fetch_nged_ltds.py
    """
    ltds_file = DATA_DIR / "nged_ltds_headroom.json"
    if not ltds_file.exists():
        print(f"  NGED LTDS: {ltds_file} not found — skipping (run fetch_nged_ltds.py first)")
        return {}
    with open(ltds_file) as f:
        recs = json.load(f)
    lookup = {}
    for key, d in recs.items():
        ...
        lookup[key] = {
            ...
            "committed_util":     d.get("committed_util_pct"),
            "committed_headroom": d.get("committed_headroom_mw"),
            "source":             d.get("source", "NGED LTDS Table 3a"),
            "quality":            "direct_headroom",
            "dno":                "NGED",
            "raw_name":           d.get("gsp_raw_name", key),
        }
    print(f"  NGED LTDS Table 3a: {len(lookup)} GSPs loaded (firm capacity − peak demand + 5-yr forecast)")
    return lookup
...
def main():
    ...
    lookups.update(load_nged())                 # AMBER - technical_limit_only baseline
    lookups.update(load_ssen())                 # AMBER (better — has demand side)
    lookups.update(load_npg())                  # GREEN - 2024 current headroom (heatmap demhr)
    lookups.update(load_ukpn())                 # GREEN - asset limit proxy (fallback)
    # BEST tier — LTDS/forecast sources run LAST so their direct_headroom +
    # committed-pipeline figures override any earlier-loaded entry under the
    # same key. This matters for cross-DNO collisions and for upgrading from
    # AMBER → GREEN sources within the same DNO.
    lookups.update(load_nged_ltds())            # BEST — NGED firm cap − peak demand + 5-yr forecast
    lookups.update(load_ssen_ltds())            # BEST — SSEN firm − peak; contracted demand as floor
    lookups.update(load_npg_ndp())              # BEST — NPg 2030 forward headroom (DFES Holistic)
    lookups.update(load_enw_heatmap())          # BEST — ENW heatmap firm demand headroom (May 2026)
    lookups.update(load_ukpn_ltds())            # BEST — UKPN firm capacity to N-1 standard
    ...
    # ── Fourth pass: NESO EA Register attachment + queue-pressure floor ──
    # The EA Register is the post-Connections-Reform "Gate 2" list of
    # connection agreements across the UK transmission network. We attach
    # per-sub aggregates (total / demand / generation / battery committed MW)
    # to every substation that name-matches, and use the demand MW as an
    # additional queue-pressure floor for substations where neither LTDS
    # nor TEC have surfaced the demand-side queue.
    ea_attached = apply_ea_register(subs)
    print(f"\n  EA Register attached: {ea_attached}/{len(subs)} subs name-matched")
    ...
    for sub in subs:
        ...
        # NESO EA Register floor — post-Gate-2 consented connection
        # agreements at this sub (gen + storage + demand combined). All of
        # it competes for substation reinforcement budget; the demand
        # subset specifically competes with a new DC load.
        ea_total_mw = sub.get("ea_total_mw")
        if ea_total_mw is not None and hr_eff > 0:
            ea_floor = round((ea_total_mw / hr_eff) * 100, 1)
            sub["ea_queue_pressure_pct"] = ea_floor
        else:
            ea_floor = None

        floors = [f for f in (ltds_floor, ea_floor) if f is not None]
        if floors:
            new_queue_pct = round(max(tec_queue_pct, *floors), 1)
        else:
